  ...
  if (mode === 'camera' && area) {
    ...
    if (recall && storedCrop) crop = clone(storedCrop);
      ...
      try {
        ...
        const { sceneItemTransform: sceneItemProperties } = await obs.getSceneItemSettings(
      ...
      try {
        ...
        await obs.configureSceneItem(
          ...
          (() => { // Visible
            ...
            /* if (config.event.online && areaName.startsWith('GameCapture') && ['sm64-psp-2p'].includes(gameLayouts.value.selected || '')) {
            // && ['DS-1p', '3DS-1p', 'sm64-psp-2p'].includes(gameLayouts.value.selected || '')) {
