...
async function updateBids() {
    try {
        const resp = await needle_1.default('get', `https://${config.address}/search/?event=${tracker_1.eventInfo[eventConfig.thisEvent - 1].id}`
            + '&type=allbids&state=OPENED', {
            cookies: tracker_1.getCookies(),
        });
        const currentBids = processRawBids(resp.body);
        replicants_1.bids.value = currentBids;
    }
    catch (err) {
        nodecg_1.get().log.warn('[Tracker] Error updating bids');
        nodecg_1.get().log.debug('[Tracker] Error updating bids:', err);
