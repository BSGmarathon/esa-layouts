    ...
    if (pin) {
        ...
        if (item) {
            ...
            const dashConfig = pin.type === 'Bid' ? {
                text: item.war ? 'Upcoming Bid War' : 'Upcoming Goal',
                fontSize: 34,
                top: 3,
            } : {
        ...
        if (next.type === 'UpcomingRun') {
            ...
            replicants_1.omnibar.value.current = Object.assign(Object.assign({}, next), { props: Object.assign(Object.assign({}, next.props), { run, dash: {
                        text: 'Up next',
                        top: 14,
                        left: 52,
                        fontSize: 50,
...
(0, nodecg_1.get)().listenFor('omnibarPlaySound', async (data, ack) => {
    if (config.obs.enabled && obs_1.default.connected) {
        try {
            ...
            if (alert && asset) {
                const source = await obs_1.default.getSourceSettings(config.obs.names.sources.donationSound);
                ...
                await obs_1.default.conn.call('SetInputVolume', {
                ...
                if (source.sourceSettings.local_file === location) {
                    await obs_1.default.conn.call('TriggerMediaInputAction', {
                        inputName: config.obs.names.sources.donationSound,
                    ...
                    await obs_1.default.setSourceSettings(config.obs.names.sources.donationSound, {
                        is_local_file: true,
                        local_file: location,
                        looping: false,
