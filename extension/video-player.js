...
sc.on('timerStopped', () => {
    ...
    if (run === null || run === void 0 ? void 0 : run.customData.intermission) {
        ...
        nodecg_1.get().log.info('[Video Player] Automatically set playlist from run data');
    }
});
...
async function playNext() {
    try {
        ...
        if (video) {
            currVideo = clone_1.default(video);
...
nodecg_1.get().listenFor('startVideoPlayer', async () => {
...
nodecg_1.get().listenFor('stopVideoPlayerEarly', () => {
    stopEarly();
    if (!obs_1.default.isCurrentScene(config.obs.names.scenes.intermission)) {
        layouts_1.obsChangeScene({ scene: config.obs.names.scenes.intermission, force: true });
    }
});
